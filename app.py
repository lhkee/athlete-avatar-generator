...
st.caption("Landmark crop + segmentation hair-safety • Pillow-first TIFF (LZW) • Streamlit Free compatible (Python 3.11).")
...
hair_safe = st.toggle("Hair-safe mode (use segmentation if available)", value=True)
...
# Loader (Pillow-first for LZW TIFF)
...
if MP_OK:
    ...
    def _hair_top_via_segmentation(pil_img, x_center, face_w, forehead_y, band_half_frac=0.35):
        try:
            ...
            with mp_seg.SelfieSegmentation(model_selection=1) as seg:
                ...
                fg = (mask > 0.35).astype(np.uint8)

            band_half = int(max(4, band_half_frac * max(face_w, 1)))
            x0 = max(0, int(x_center) - band_half)
            x1 = min(W - 1, int(x_center) + band_half)
            band = fg[:, x0:x1]

            start = int(max(forehead_y - 0.50 * H, 0))
    ...
    def _crop_from_landmarks(pil_img, face_box, landmarks, target_label, kind):
        ...
        margin = (0.020 if kind == "avatar" else 0.016) * th
                ...
                if debug:
                    ...
                    if dbg is not None:
                        try:
                            ...
                            cv2.line(pr, (0, out_top), (w-1, out_top), (0,255,255), 1)
                        except Exception:
                            pass
